...
#    with open(model, 'rb') as f:
    with open('./MOVIDIUS/graph', 'rb') as f:
        graphfile = f.read()
    ...
    #        i = 0
            ...
            cap_height,  cap_width, cap_depth = c_frame.shape
            cap_rate = 0.5
...
#            print(size)
            ...
            mean = 128
            std = 1/128
            colorimg = cv2.cvtColor(c_frame, cv2.COLOR_BGR2RGB)
            for i in range(3):
                colorimg[:,:,i] = (colorimg[:,:,i] - mean) * std


            print('Start download to NCS...')
            graph.LoadTensor(colorimg.astype(np.float16), 'user object')
