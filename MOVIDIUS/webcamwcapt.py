...
#        self.window.attributes("-fullscreen",True)
        ...
        ### frame in frame
        self.btn_f = tkinter.Frame(window, relief=tkinter.RIDGE, bd=2)
        self.text_form = tkinter.Entry(self.btn_f)
        self.text_form.grid(row=0, column=0)
        # Button that lets the user take a snapshot
        self.btn_snapshot=tkinter.Button(self.btn_f, text="Snapshot", width=50, command=self.snapshot)
        self.btn_snapshot.grid(row=1, column=0) #.pack(anchor=tkinter.CENTER, expand=True)
    ...
    def snapshot(self):
        ...
        if ret:
            _, frame = self.f_net.face_rec(ret, frame, self.top_name)
...
#            cv2.imwrite("frame-" + time.strftime("%d-%m-%Y-%H-%M-%S") + ".jpg", cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
    ...
    def update(self):
        # Get a frame from the video source
        ret, frame = self.vid.get_frame()
        self.top_name, frame = self.f_net.face_rec(ret, frame, self.top_name)
        frame = cv2.resize(frame, (self.x, self.y))
        if ret:
...
#    cv2.waitKey(0)
    ...
    def face_rec(self, ret, c_frame, cullent_name):
        #size = (320,240)
